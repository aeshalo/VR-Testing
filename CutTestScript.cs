...
public class CutTestScript : MonoBehaviour {
    ...
    void Start () {
        ...
        CuttingPlane = new Plane(GameObject.Find("Cutter").transform.rotation * new Vector3(0, 0, -1), GameObject.Find("Cutter").transform.position);
    ...
    {
        //get references to origional mesh data
        ...
        //find out which vertex is on which side of the cut and add add to respective list
        ...
        for (int i = 0; i < tris.Length; i += 3)
        {
            ...
            //add trangles that are all above the plane to trisLower
            ...
            //if neither, then tri cuts the plane. therefore "cut" and figure ourt the correct new tri/tris and generate all their data(pos,uv etc) FORNOW above only
            else
            {
                ...
                NewTriEdgeVerticiesUpper[3] = 65565; //temp value, if left unchanged then NTEVU[4] has not been used. a brachdepends on this later so certainty is neccesary (0 might get used in some circumstances)
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    if (VertexAbovePlane[tris[i + j]] != VertexAbovePlane[tris[i + (j + 1) % 3]])
                    {
                        ...
                        //Find the point on the intersecting edge that meets the plane
                        float distance;
